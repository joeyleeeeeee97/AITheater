...
class GameMasterResume:
    ...
    async def resume_and_finish_game(self):
        ...
        await self._initialize_agents_for_assassination()
        await self._run_resumed_assassination_phase()
        
        # Stage 2: MVP
        await self._initialize_remaining_agents_for_mvp()
    ...
    async def _initialize_agents_for_assassination(self):
        game_logger.info("--- Initializing Evil Team for Assassination ---")
        evil_for_discussion_ids = [pid for pid, role in self.roles.items() if role in ["Assassin", "Morgana", "Mordred"]]
        
        await self._initialize_and_brief_agents(evil_for_discussion_ids)
        game_logger.info("Evil team initialized and briefed.")

    async def _initialize_remaining_agents_for_mvp(self):
        game_logger.info("--- Initializing Remaining Agents for MVP Phase ---")
        existing_ids = {agent.player_id for agent in self.agents}
        remaining_ids = [pid for pid in range(self.num_players) if pid not in existing_ids]
        
        await self._initialize_and_brief_agents(remaining_ids)
        self.agents.sort(key=lambda x: x.player_id)
        game_logger.info("All agents are now initialized and briefed.")
    ...
    async def _initialize_and_brief_agents(self, player_ids: List[int]):
        ...
        for player_id in player_ids:
            ...
            await agent.receive_message(review_msg) # We send the request and let it prime the agent's context
